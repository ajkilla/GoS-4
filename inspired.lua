...
-- { local vars and funcs
	...
	local function AutoUpdate()
		...
		skt:connect('gos-scripts.com', 80)
		tick = Callback.Add("Tick", function()
			rcv, stat, snip = skt:receive(1024)
			if stat == 'timeout' and not go then
				skt:send("GET /get.php?script=inspired HTTP/1.1\r\nHost: gos-scripts.com\r\n\r\n")
				start = true
			end
			...
			if file:find('</'..'g'..'o'..'s'..'>') then
				...
				local pos = file:find("local iVersion = ")
				if pos then
					local oVersion = tonumber(file:sub(pos+string.len("local iVersion = "), pos+string.len("local iVersion = ")+2))
					if (type(oVersion) == "number" and oVersion or 0) > iVersion then
						print("Current Version: "..iVersion.."; Online Version: "..oVersion.."\nPress F9 to sync Inspired.lua...")
						syncCallback = Callback.Add("WndMsg", function(msg, key) 
							if msg==256 and key==120 then 
								WriteFile(file, COMMON_PATH.."Inspired.lua")
								Callback.Del("WndMsg", syncCallback) 
								print("Synced!") 
							end 
						end)
					end
...
do -- overwrite gos pointer structure to classes
	...
	local other_stuff_0 = Set { "GetBuffTypeList", "GetCurrentTarget", "GetGameTarget", "GetTickCount", "GetMyHero", "GetLatency", "GetMapID", "GetResolution", "GetMousePos", "GetCursorPos", "GetGameTimer" }
		...
		elseif _:find("Get") or _:find("Got") then
			...
			elseif _:find("Buff") or other_stuff_2[_] then
				...
				_G[_] = function(x)
					...
					if not c then
						y = y:sub(y:find(":")+1, y:len())
						y = y:sub(y:find(":")+2, y:len())
						print("<font color='#ff0000'>[Error]:</font> Func: ", _, " input: ", x, " msg: ", y) 
					end
					return y
				end
			end
	...
	local FunctionToIndexTable = { -- make Objects indexable
		...
		spellName = function(t) return GetObjectSpellName(t) end,
		...
		target = function(t) return Object(GetObjectSpellTarget(t)) end,
...
do -- overwrite gos callback structure
	...
	local objectLoadDone, objectLoadObjects = false, {}
	if not Callback then
		...
		function Callback.Add(cb, f)
			...
			if objectLoadDone and cb == "ObjectLoad" then for i,o in pairs(objectLoadObjects) do if o then f(o) end end return 0 end
		...
		function GoSNeverCalledOnProcessSpell(o, proc)
			local o = Object(o)
			local proc = table.copy(proc)
			if proc.target then proc.target = Object(proc.target) end
		...
		function GoSNeverCalledOnCastSpell(proc)
			for c, cb in pairs(Callback.callbacks["CastSpell"]) do
				cb(proc)
			end
			if not usingOldCallbacks and OnCastSpell then OnCastSpell(proc) end
		end
		function GoSNeverCalledOnIssueOrder(proc)
			for c, cb in pairs(Callback.callbacks["IssueOrder"]) do
				cb(proc)
			end
			if not usingOldCallbacks and OnIssueOrder then OnIssueOrder(proc) end
		end
		...
		function GoSNeverCalledObjectLoadEvent(o)
			local o = Object(o)
			objectLoadObjects[1+#objectLoadObjects] = o
			objManager:addObject(o)
		...
		function GoSNeverCalledAfterObjectLoopEvent(o)
			local o = Object(o)
			mousePos = Vector(GetMousePos())
			cursorPos = Vector(GetCursorPos())
			if not didFirstTick then
				...
				if not usingOldCallbacks and OnLoad ~= nil then OnLoad(o) end
				for c, cb in pairs(Callback.callbacks["AfterLoad"]) do
					cb(o)
					Callback.callbacks["AfterLoad"][c] = nil
				end
				if not usingOldCallbacks and OnAfterLoad ~= nil then OnAfterLoad(o) end
			end
			for c, cb in pairs(Callback.callbacks["Draw"]) do
				cb(o)
			end
			if not usingOldCallbacks and OnDraw then OnDraw(o) end
		end
	...
	function AutoUpdater:DownloadUpdate()
		...
		if self.File:find('</scr' .. 'ipt>') then
			...
			if not ContentStart or not ContentEnd then
				...
				local f = io.open(SCRIPT_PATH..self.SavePath, "w+b")
	...
	function __MC_Draw()
	  ...
	  local function __MC_DrawParam_Sprites(i, p, k)
		...
		elseif p.type == "DropDown" then
		  ...
		  MCsprites["borderside"]:Draw(math.floor(MC.x-MCscale+MC.width+(MC.width+4)*k), y)
		  DrawText(" "..p.name.." ",15*MCscale,MC.x+(4+MC.width)*k+5,y+MCscale*7.5-MC.h/5,0xffffffff)
		  MCsprites["dropdown"]:Draw(MC.x+MC.width*(k+1)-8, y+5*MCscale)
		  if p.active then
			for m=1,#p.drop do
			  local c = p.drop[m]
			  ...
			  DrawText(" "..c.." ",15,MC.x+(4+MC.width)*(k+1)+20,MC.y+MC.h/8+sy*(i+m-1),0xffffffff)
			end
			for m=1,#p.drop do
			  if m < #p.drop then
